...
def parse_args():
    ...
    # Core DQN parameters
    parser.add_argument("--replay-buffer-size", type=int, default=int(1e3), help="replay buffer size")
...
with U.make_session(120) as sess:
    ...
    # Main training loop
    head = np.random.randint(args.num_heads)        #Initial head initialisation
    log_flag = False
    disp_niter_freq=1000
    print('start playing ...')
    while True:
        num_iters += 1
        if (num_iters % disp_niter_freq)==0:
        ...
        if (num_iters > max(0.5 * args.batch_size, args.replay_buffer_size) and
            ...
            alpha=learning_rate.value(num_iters)
            td_errors = train(obses_t, actions, rewards, obses_tp1, dones, weights,alpha)
